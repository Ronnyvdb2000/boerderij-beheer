...
app.get('/api/mestbank/:klantnr', async (req, res) => {
  ...
  try {
    const [
      ...
      // Stalbezetting: enkel het laatste jaar dat voor deze klant beschikbaar is.
      pool.query(
        `SELECT * FROM "10 mestbank dieren stalbezetting"
         WHERE klantnr = $1 AND jaar = (SELECT MAX(jaar) FROM "10 mestbank dieren stalbezetting" WHERE klantnr = $1)
         ORDER BY id`,
        [klantnr]
      ),
      pool.query(`SELECT * FROM "10 voorw rv" WHERE klantnr = $1 AND jaar = $2`, [klantnr, HUIDIG_JAAR_MIN_1]),
      // Mestbank dieren: laatste + vorige jaar dat voor deze klant beschikbaar is.
      pool.query(
        `SELECT d.*, dr.diernaam FROM "10 mestbank dieren" d LEFT JOIN "21 dieren" dr ON d.diercode = dr.diercode
         WHERE d.klantnr = $1 AND d.jaartal IN (
           SELECT DISTINCT jaartal FROM "10 mestbank dieren" WHERE klantnr = $1 ORDER BY jaartal DESC LIMIT 2
         )
         ORDER BY d.jaartal DESC`,
        [klantnr]
      ),
      // Stock voeder: laatste + vorige jaar dat voor deze klant beschikbaar is.
      pool.query(
        `SELECT sv.*, dr.diernaam FROM "12 mestbank stock voeder" sv LEFT JOIN "21 dieren" dr ON sv.diercode_stock = dr.diercode
         WHERE sv.klantnr = $1 AND sv.jaartal IN (
           SELECT DISTINCT jaartal FROM "12 mestbank stock voeder" WHERE klantnr = $1 ORDER BY jaartal DESC LIMIT 2
         )
         ORDER BY sv.jaartal DESC`,
        [klantnr]
      ),
      // Andere voeders: laatste + vorige jaar dat voor deze klant beschikbaar is.
      pool.query(
        `SELECT * FROM "14 voeders"
         WHERE klantnr = $1 AND jaar IN (
           SELECT DISTINCT jaar FROM "14 voeders" WHERE klantnr = $1 ORDER BY jaar DESC LIMIT 2
         )
         ORDER BY jaar DESC`,
        [klantnr]
      )
